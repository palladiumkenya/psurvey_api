...
def current_user(request):
    if request.method == "GET":
        ...
        if request.user.access_level.id == 1:
            ...
            aq = Questionnaire.objects.filter(is_active=True, is_published=True, active_till__gte=date.today()).order_by('-created_at').count()
            ...
            aq = Questionnaire.objects.filter(id__in=q, is_active=True, is_published=True, target_app='Facility', active_till__gte=date.today()).order_by('-created_at').count()
