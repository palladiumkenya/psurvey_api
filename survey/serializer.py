...
class QuestionSetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Question
        fields = ('Question_ID', 'QuestionName',
...
class QuestionAnswDepDataSerializer(serializers.ModelSerializer):
    ...
    def get_questionnaire_data(self, obj):
        ...
        if obj.has_uploaded_data:
            q_id = obj.id
            mfl_code = self.context["mfl_code"]
            query_set = Questionnaire_Data.objects.filter(
                questionnaire_id=q_id, mfl_code=mfl_code)
            q_data = QuestionnaireDataSerializer(query_set, many=True).data
